			...
			<div class="span12 box-content">
				<div class="span12">
						<div class="well">
							...
							<br>
							<h3>ID PRE-ORDER: {{$pengaturan->invoice}}{{$preorder->kodePreorder}}</h3>Total Harga Belanjaan: {{jadiRupiah($preorder->total)}}<hr>
							Total yang harus dibayar diawal : {{jadiRupiah($preorder->dp)}}<hr>
				...
				@if($datapembayaran=='1')
						<div class="span12">
							<div class="well">
								...
								<a href="{{URL::to('konfirmasiorder/'.$preorder->id)}}" class="btn theme">Konfirmasi Pembayaran</a>
							</div>
						</div>
					@endif
					@if($datapembayaran=='2')
					...
					@if($datapembayaran=='4')
						...
						<a class="btn btn-info" href="{{URL::to('ipaymu/'.$preorder->id)}}" target="_blank">Bayar dengan iPaymu</a>
					@endif
			</div>
		</div>
		</div>
